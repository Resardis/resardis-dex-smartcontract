...
contract MatchingEvents {
    ...
    event LogMinSell(address payGem, uint minAmount);
...
contract MatchingMarket is MatchingEvents, ExpiringMarket, DSNote {
    ...
    //
    function offer(
        uint payAmt,    //maker (ask) sell how much
    ...
    function getBestOffer(address sellGem, address buyGem) public view returns(uint) {
    ...
    function getOfferCount(address sellGem, address buyGem) public view returns(uint) {
    ...
    function sellAllAmount(address payGem, uint payAmt, address buyGem, uint minFillAmount)
        public
        returns (uint fillAmt)
    {
        require(!_locked, "Reentrancy attempt");
        uint offerId;
        while (payAmt > 0) {                           //while there is amount to sell
            ...
            if (payAmt >= offers[offerId].buyAmt) {                       //If amount to sell is higher or equal than current offer amount to buy
                ...
                uint256 baux = rmul(payAmt * 10 ** 9, rdiv(offers[offerId].payAmt, offers[offerId].buyAmt)) / 10 ** 9;
    ...
    function buyAllAmount(address buyGem, uint buyAmt, address payGem, uint maxFillAmount)
        public
        returns (uint fillAmt)
    {
        require(!_locked, "Reentrancy attempt");
        ...
        while (buyAmt > 0) {                           //Meanwhile there is amount to buy
            ...
            if (buyAmt >= offers[offerId].payAmt) {                       //If amount to buy is higher or equal than current offer amount to sell
                ...
                fillAmt = add(fillAmt, rmul(buyAmt * 10 ** 9, rdiv(offers[offerId].buyAmt, offers[offerId].payAmt)) / 10 ** 9); //Add amount sold to acumulator
    ...
    function getBuyAmount(address buyGem, address payGem, uint payAmt) public view returns (uint fillAmt) {
        ...
        fillAmt = add(fillAmt, rmul(payAmt * 10 ** 9, rdiv(offers[offerId].payAmt, offers[offerId].buyAmt)) / 10 ** 9); //Add proportional amount of last offer to buy accumulator
    }

    function getPayAmount(address payGem, address buyGem, uint buyAmt) public view returns (uint fillAmt) {
        uint256 offerId = getBestOffer(buyGem, payGem);           //Get best offer for the token pair
        ...
        fillAmt = add(fillAmt, rmul(buyAmt * 10 ** 9, rdiv(offers[offerId].buyAmt, offers[offerId].payAmt)) / 10 ** 9); //Add proportional amount of last offer to pay accumulator
    }
        ...
        returns (bool)
    {
        ...
        if (isActive(id) && offers[id].payAmt < _dust[address(offers[id].payGem)]) {
    ...
    function _isPricedLtOrEq(
        ...
        returns (bool)
    {
        return mul(offers[low].buyAmt, offers[high].payAmt)
    ...
    function _matcho(
        uint tPayAmt,    //taker sell how much
        address tPayGem,   //taker sell which token
        uint tBuyAmt,    //taker buy how much
        ...
        returns (uint id)
    {
        ...
        uint tBuyAmtOld;    //taker buy how much saved
        uint mBuyAmt;        //maker offer wants to buy this much token
        uint mPayAmt;        //maker offer wants to sell this much token
        ...
        while (_best[address(tBuyGem)][address(tPayGem)] > 0) {
            ...
            if (mul(mBuyAmt, tBuyAmt) > mul(tPayAmt, mPayAmt) +
        ...
        internal
    {
        ...
        pos = pos == 0 || offers[pos].payGem != payGem || offers[pos].buyGem != buyGem || !isOfferSorted(pos)
        ?
            _find(id)
        :
            _findpos(id, pos);
        ...
        returns (bool)
    {
        ...
        if (id != _best[payGem][buyGem]) {              // offers[id] is not the highest offer
