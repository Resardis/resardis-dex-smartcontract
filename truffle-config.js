...
module.exports = {
  ...
  networks: {
    ...
    loom_local: { // eslint-disable-line
      provider: function () {
        const chainId = 'default';
        const writeUrl = 'http://127.0.0.1:46658/rpc';
        const readUrl = 'http://127.0.0.1:46658/query';
        const privateKeyPath = path.join(__dirname, `${privateDir}/loom_local_private_key`);
    ...
    loom_extdev_plasma_us1: { // eslint-disable-line
      provider: function () {
        const chainId = 'extdev-plasma-us1';
        const writeUrl = 'http://extdev-plasma-us1.dappchains.com:80/rpc';
        const readUrl = 'http://extdev-plasma-us1.dappchains.com:80/query';
        const privateKeyPath = path.join(__dirname, `${privateDir}/loom_extdev_private_key`);
        if (fs.existsSync(privateKeyPath)) {
          const loomTruffleProvider = getLoomProviderWithPrivateKey(privateKeyPath, chainId, writeUrl, readUrl);
          loomTruffleProvider.createExtraAccountsFromMnemonic(dummyMnemonic, 15);
          return loomTruffleProvider;
    ...
    loom_mainnet: { // eslint-disable-line
      provider: function () {
        ...
        const privateKeyPath = path.join(__dirname, `${privateDir}/loom_mainnet_private_key`);
        if (fs.existsSync(privateKeyPath)) {
          const loomTruffleProvider = getLoomProviderWithPrivateKey(privateKeyPath, chainId, writeUrl, readUrl);
          return loomTruffleProvider;
        } else {
