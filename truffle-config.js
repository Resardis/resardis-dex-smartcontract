  ...
  networks: {
    ...
    rinkeby: {
      provider: function () {
        ...
        const privateKeyPath = path.join(__dirname, `${privateDir}/rinkeby_private_key`);
        if (fs.existsSync(privateKeyPath)) {
          const privateKey = readFileSync(path.join(__dirname, `${privateDir}/rinkeby_private_key`), 'utf-8');
  ...
  compilers: {
    solc: {
      ...
      settings: { // See the solidity docs for advice about optimization and evmVersion
        optimizer: {
          enabled: true,
          // @TODO: increase the optimizer as much as possible (256 bit integer?)
          runs: 1000,
        },
