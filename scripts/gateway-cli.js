...
async function depositEthToRinkebyGateway (web3js, amount, unit, ownerAccount) {
  ...
  async function sendSigned (txData, cb) {
    ...
    const txReceipt = await web3js.eth.sendSignedTransaction('0x' + serializedTx, cb);
    return txReceipt;
  }
  ...
  .action(async function (options) {
    ...
    try {
      ...
      if (receipt.tokenContract.toString() === myRinkebyCoinAddress.toString()) {
        ...
        console.log(`${amountInWei.toString()} wei (${amountInEth.toString()} in eth) withdrawn from Eth Gateway.`);
        console.log(`Rinkeby tx hash: ${txHash}`);
      } else {
    ...
    try {
      ...
      if (options.chain === 'eth') {
        ...
        try {
          ...
          console.log('Error while getting the ERC20 balance from the PlasmaChain.');
          throw err;
        } finally {
          client.disconnect();
        }
  ...
  .action(async function (options) {
    try {
      ...
      if (options.chain === 'eth') {
        ...
        const { account, web3js, client } = loadExtdevAccount(); // eslint-disable-line
        ownerAddress = account;
        if (options.account) {
          ownerAddress = (options.account === 'gateway') ? extdevGatewayAddress : options.account;
        }
        try {
          ...
          console.log('Error while getting the ETH balance from the PlasmaChain.');
          throw err;
        } finally {
          client.disconnect();
        }
      }
      const balanceInEth = (new BN(balance).div(new BN(10).pow(new BN(18)))).toString();
    ...
    try {
      ...
      if (options.chain === 'eth') {
        ...
        try {
          ...
          console.log('Error while getting the ERC721 balance from the PlasmaChain.');
          throw err;
        } finally {
          client.disconnect();
        }
